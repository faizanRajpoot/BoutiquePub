...
function HeaderPopUp() {
    ...
    useEffect(() => {
        axios
            .get("http://127.0.0.1:4000/header")
            .then((res) => setHeaderData(res?.data?.result));
    }, [])


    useEffect(() => {
        const filter = headerData?.find((v) => v.id === Number(headerparams.id));
        setFilteredHeader(filter);
    }, [headerData])


    function headerCart() {
        axios.post("http://127.0.0.1:4000/headerCheckout",filteredHeader)
    };

    return (
        <div className="w-full  ">
            <div className="w-full h-44 bg-[#F6F6F6] flex flex-col justify-center items-center md:h-48 sm:h-32">
                <h1 className="text-5xl font-semibold mb-5 lg:text-3xl md:text-2xl">{filteredHeader?.name}</h1>
                <h1 className="text-xs tracking-widest">
                    Home / <span className="text-red-600"> {filteredHeader?.name} </span>
                </h1>
            </div>
            <div className="w-full h-[700px]  flex lg:h-[650px] lgg:h-[600px] md:flex md:flex-col md:h-full md:pb-16 ">
                <div className="w-1/2 h-full  py-12 pl-24 pr-0 md:w-full md:h-[750px] md:px-5 md:py-0 md:pt-5  xs:h-[500px]">
                    <img src={filteredHeader?.img} alt="Blank" className="w-full h-full" />
                </div>
                <div className="w-1/2 h-full py-12 px-5 pr-10 md:py-2 md:w-full">
                    <p className="text-xl font-medium"> {filteredHeader?.name} </p>
                    <p className="mt-20 lgg:mt-7 md:mt-20"> Rs:{filteredHeader?.price} </p>
                    ...
                    <div className="flex flex-col md:w-full md:flex md:flex-row md:items-center md:justify-between md:mt-5  ">
                        <div className="flex items-center w-2/3  justify-between lg:w-full  lg:flex lg:justify-between lg:items-center md:w-3/5 sm:w-4/7 xs:w-3/5">
                            ...
                            <button onClick={headerCart} className="w-48 h-16 border-2 border-[#e73333] text-[#e73333] uppercase text-xs font-bold flex justify-center items-center lgg:w-36 sm:w-28 sm:text-[10px] xs:w-20 xs:text-[7px]">
