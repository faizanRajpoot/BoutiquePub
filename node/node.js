...
app.post("/headerCheckout", (req, res) => {
  const body =req.body;
  return connection.query(
    `insert into headerCheckout (categ,name,brand,img,price) values(?,?,?,?,?)`,
    [body.categ, body.name, body.brand, body.img, body.price],
    (err, result) => {
      console.log(err);
      if (err) {
        return res.status(500).json({ message: "somthing error" });
...
app.get("/header", (req, res) => {
  return connection.query(`select * from header`, [], (err, result) => {
    if (err) {
      return res.status(500).json({ message: "error" });
    }
    return res.json({ message: "success", result });
...
app.get("/headerWomen", (req, res) => {
  return connection.query(`select * from header where categ = "Women"`, [], (err, result) => {
    if (err) {
      return res.status(500).json({ message: "error" });
    }
    return res.json({ message: "success", result });
...
app.get("/headerMen", (req, res) => {
  return connection.query(`select * from header where categ = "Mens"`, [], (err, result) => {
    if (err) {
      return res.status(500).json({ message: "error" });
    }
    return res.json({ message: "success", result });
